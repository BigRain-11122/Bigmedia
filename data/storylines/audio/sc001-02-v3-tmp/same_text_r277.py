...
# R275 same-text checker adapted for ch.2 v3 (source change only + ch.2 anchors)
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
novel = io.open(os.path.join(ROOT, "data", "storylines", "novel", "SC-001-02-v3.md"), encoding="utf-8").read()
beats_raw = io.open(os.path.join(ROOT, "data", "storylines", "audio", "SC-001-02-v3.beats.txt"), encoding="utf-8").read()
...
a1_ok = beats[1][0] == "punch" and "一万零三个人的口味" in beats[1][2]
...
x15_ok = (len(vulgar) == 0) and ("数据流" in content_all) and ("系统日志" in content_all)

with io.open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "same-text-check-r277.txt"), "w", encoding="utf-8") as f:
