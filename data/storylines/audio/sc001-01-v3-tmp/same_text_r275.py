...
novel = io.open(os.path.join(ROOT, "data", "storylines", "novel", "SC-001-01-v3.md"), encoding="utf-8").read()
beats_raw = io.open(os.path.join(ROOT, "data", "storylines", "audio", "SC-001-01-v3.beats.txt"), encoding="utf-8").read()
...
# O-1756 x1.5 register inheritance spot-checks on same-text source (v3-only, v2 had these vulgar words)
vulgar = [w for w in ["发疯", "枪毙", "卧槽", "杀疯了", "燃爆", "封神"] if w in content_all]
x15_ok = (len(vulgar) == 0) and ("系统架构决策" in content_all) and ("对自己不留情面" in content_all)

with io.open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "same-text-check-r275.txt"), "w", encoding="utf-8") as f:
