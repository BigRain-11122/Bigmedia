...
# R279 same-text checker adapted for ch.4 v3 (source change only + ch.4 anchors)
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
novel = io.open(os.path.join(ROOT, "data", "storylines", "novel", "SC-001-04-v3.md"), encoding="utf-8").read()
beats_raw = io.open(os.path.join(ROOT, "data", "storylines", "audio", "SC-001-04-v3.beats.txt"), encoding="utf-8").read()
...
hook = beats[0][2]
hook_ok = ("AI 参与生成" in hook) and ("真实事件改编" in hook) and ("来源清单见图文页" in hook) and ("第四章" in hook)

# A1 hook-position check: cue01 declaration (compliance-mandated) then golden-100 opening
a1_ok = beats[1][0] == "punch" and "给失败立的" in beats[1][2]
...
x15_ok = (len(vulgar) == 0) and ("全城大编译" in content_all) and ("数据清洗" in content_all)

with io.open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "same-text-check-r281.txt"), "w", encoding="utf-8") as f:
