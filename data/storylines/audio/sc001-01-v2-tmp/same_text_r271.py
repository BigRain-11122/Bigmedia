...
novel = io.open(os.path.join(ROOT, "data", "storylines", "novel", "SC-001-01-v2.md"), encoding="utf-8").read()
beats_raw = io.open(os.path.join(ROOT, "data", "storylines", "audio", "SC-001-01-v2.beats.txt"), encoding="utf-8").read()

# body paragraphs = between first and second '---' (v2 layout: header/table before 1st, sources after 2nd)
parts = novel.split("\n---\n")
body = parts[1]
...
# A1 hook-position check: first 30s = cue01 declaration (compliance-mandated) then golden-100 opening
a1_ok = beats[1][0] == "punch" and "员工总数：零" in beats[1][2]

with io.open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "same-text-check-r271.txt"), "w", encoding="utf-8") as f:
...
print("paras=%d beats=%d miss=%d cta_ok=%s hook_ok=%s a1_ok=%s" % (len(paras), len(beats), len(miss), cta_ok, hook_ok, a1_ok))
sys.exit(1 if (miss or not cta_ok or not hook_ok or not a1_ok) else 0)
