# -*- coding: utf-8 -*-
"""R397 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 3/6 (window R395-R400) -> no commit this round per os-protocol S6."""
import json, io, os
...
FOCUS = (
    u"R398: #21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
    ...
    u"新令/集团转办/探针红出现即优先（异常或实活轮出现即提前收账）；全静即 idle-fast（4/6）"
)

line = (
    log_ts + " R397: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 3/6 不 commit）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R395/R396 idle-fast 并窗自记账预期态·"
    u"?? .c3-tmp/r395_close.py+r396_close.py+r397_close.py 自产脚本件随窗满批 commit〔窗 R395-R400〕·"
    u"无 index.lock r359_check.py False 实证·HEAD=7655697 未变·7655697 后零插队=无 bm-a 活跃写盘迹象）"
    ...
    u"〔12 log-order+8 heartbeat-gap〕·tick396=done396 对账平）；"
    ...
    u"一行收账即出（idle-fast 并窗轮 3/6〔窗 R395-R400 满 6 收账〕·本轮不 commit·P-61 导出步照刷 export_ts）。"
    u"下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/"
...
assert state["tick"] == 396, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R397:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R396:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 397 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R398:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R397 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
