# -*- coding: utf-8 -*-
# R362 idle-fast close (window round 1/6, NO commit this round):
...
entry = (
    u"2026-09-26 %s %s: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 1/6 不 commit）——"
    ...
    u"③树态=仅自产预期件（R361 窗满 batch commit 14b7d22 后轮首净态·git status 空输出+无 index.lock python os.path.exists False 实证"
    u"·本轮 M state.json+M status-export.json=R362 自记账预期态+?? .c3-tmp/r362_close.py 收账件随窗满批 commit〔窗 R362-R367〕）"
    ...
    u"/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick361=done361 对账平）；"
    ...
    u"——一行收账即出（idle-fast 并窗轮 1/6〔窗 R362-R367 满 6 收账〕·本轮不 commit·P-61 导出步照刷 export_ts）。"
    u"下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——全静即 idle-fast（2/6）。"
) % (now.strftime("%H:%M"), R)
...
assert old_tick == 361, "unexpected tick %d" % old_tick
...
assert data["tick"] == 362, "tick mismatch"
assert data["log"][-1].endswith(u"全静即 idle-fast（2/6）。"), "entry tail mismatch"
assert len(data["log"]) == 374, "log count unexpected: %d" % len(data["log"])
...
print("OK tick=362 ts=%s task_len=%d export_ts=%s" % (stamp_full, len(task), ts_iso))
