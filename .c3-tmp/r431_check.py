# -*- coding: utf-8 -*-
# r431 real-work round five-check (r429_check.py pattern) + three probes
# round task: #69 P-2026-09-26-07 fleet model share (bigstream) Phase1 self-check
...
# 0b) bm-a insert commits after 51042d2 (R430 anchor)
r = subprocess.run(["git", "log", "--oneline", "51042d2..HEAD"], capture_output=True, text=True, encoding="utf-8", errors="replace")
inter = [x for x in (r.stdout or "").splitlines() if x.strip()]
log("inserts_after_51042d2=%d" % len(inter))
if inter:
    for x in inter[:5]:
        log("insert=" + x[:100])

# 1) group ledger @-pattern count (anchor=25 set by R429 three new orders; tail P-2026-09-26-08)
...
out_path = os.path.join(repo, ".c3-tmp", "r431_probe.txt")
...
with io.open(os.path.join(repo, ".c3-tmp", "r431_check.txt"), "w", encoding="utf-8") as f:
    f.write("\n".join(L) + "\n")
print("check_file=.c3-tmp/r431_check.txt")
