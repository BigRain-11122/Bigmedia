# -*- coding: utf-8 -*-
"""R389 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 1/6 (window R389-R394) -> no commit this round per os-protocol S6."""
import json, io, os
...
FOCUS = (
    u"R390: #21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
    ...
    u"新令/集团转办/探针红出现即优先；全静即 idle-fast（并窗 2/6）"
)

line = (
    log_ts + " R389: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 1/6 不 commit）——"
    ...
    u"③树净零锁（HEAD=8d0ba25 R383-R388 batch 收账后净态·git status 空输出+无 index.lock r359_check.py "
    u"False 实证·本轮自产 M state.json+M status-export.json+?? .c3-tmp/r389_close.py=并窗自记账预期态"
    u"非 bm-a 迹象〔随窗满批 commit〕）"
    ...
    u"〔12 log-order+8 heartbeat-gap〕·tick388=done388 对账平）；"
    ...
    u"一行收账即出（idle-fast 并窗轮 1/6〔窗 R389-R394 满 6 收账〕·本轮不 commit·"
...
assert state["tick"] == 388, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R389:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R388:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 389 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R390:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R389 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
