# -*- coding: utf-8 -*-
# r428 fast-path five-check (r427_check.py pattern) + three probes
...
# 0b) bm-a insert commits after 08812b3 (R425 anomaly-close anchor, window R426-R431 not yet committed)
r = subprocess.run(["git", "log", "--oneline", "08812b3..HEAD"], capture_output=True, text=True, encoding="utf-8", errors="replace")
inter = [x for x in (r.stdout or "").splitlines() if x.strip()]
log("inserts_after_08812b3=%d" % len(inter))
if inter:
    for x in inter[:5]:
        log("insert=" + x[:100])
...
out_path = os.path.join(repo, ".c3-tmp", "r428_probe.txt")
...
with io.open(os.path.join(repo, ".c3-tmp", "r428_check.txt"), "w", encoding="utf-8") as f:
    f.write("\n".join(L) + "\n")
print("check_file=.c3-tmp/r428_check.txt")
