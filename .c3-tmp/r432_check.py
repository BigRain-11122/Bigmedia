# -*- coding: utf-8 -*-
# r432 five-check (r431_check.py pattern, HEAD anchor 7927595=R431 close commit) + three probes
# round focus: #70 P-2026-09-26-08 oss-harvest first window slice (claimable top row)
...
# 0b) bm-a insert commits after 7927595 (R431 close commit anchor)
r = subprocess.run(["git", "log", "--oneline", "7927595..HEAD"], capture_output=True, text=True, encoding="utf-8", errors="replace")
inter = [x for x in (r.stdout or "").splitlines() if x.strip()]
log("inserts_after_7927595=%d" % len(inter))
if inter:
    for x in inter[:5]:
        log("insert=" + x[:100])

# 1) group ledger @-pattern count (anchor=25 set by R429; tail P-2026-09-26-08)
...
out_path = os.path.join(repo, ".c3-tmp", "r432_probe.txt")
...
# 9) oss-harvest ledger dir existence (HQ repo, read-only check for #70 window bookkeeping)
oh_dir = os.path.join(grp, "cph4", "oss-harvest")
log("oss_harvest_dir=%s" % os.path.isdir(oh_dir))
if os.path.isdir(oh_dir):
    log("oss_harvest_files=%s" % ",".join(sorted(os.listdir(oh_dir))[-5:]))

with io.open(os.path.join(repo, ".c3-tmp", "r432_check.txt"), "w", encoding="utf-8") as f:
    f.write("\n".join(L) + "\n")
print("check_file=.c3-tmp/r432_check.txt")
