# R349 idle-fast close: state.json (tick 348->349, log append, ts/task refresh)
# + P-61 export step (status-export.json export_ts/depts/outs/results derived refresh)
# Window 6/6 FULL -> batch commit R344-R349 this round (os-protocol S6 batching rule).
# Pattern reuse: r348_close.py (R322 lesson: state.json multi-line edits via one-shot python).
...
log_prefix = now.strftime("%Y-%m-%d %H:%M") + " R349: "

BODY = (
    "idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗 6/6 满=本窗 batch commit R344-R349）——"
    "①无新令（orders 顶=O-20260925-1931-HQ-C·R283 已记账）"
    "②backlog 顶行不可认领（#63 图鉴续件 C-00030 锚正典位轮首核=仍不在位〔python os.path.exists False+C-00031 同核 False 实证·anchors 尾二=C-00028/C-00029 止位〕supply-gated 维持·"
    ...
    "③树态=仅自产预期件（M state.json+M status-export.json=R344-R348 idle-fast 并窗自记账预期态·?? .c3-tmp/r344~r349 六自产脚本件随本窗满批 commit·"
    "无 index.lock python os.path.exists False 实证·HEAD=b7e43f4）"
    "④ch.5 v3 稿未落（storylines python walk 阈值复算实证 novel/audio/comic 三子域 09-26 零新写盘〔首轮 epoch 笔误 1761715200=2025 年致 502 件伪命中即纠·正确阈值 1790352000=0 件〕=ch.5 判定口径成立·bm-a 面）·"
    ...
    "三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0/readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick348=done348 对账平）；"
    ...
    "窗满 6 轮触发=batch commit R344-R349（os-protocol §6 并窗律·commit 注区间·窗重置 1/6）·P-61 导出步照刷 export_ts。"
...
ENG_T = (
    "OS 循环 R349（idle-fast·五静+探针绿——产线全 supply-gated 维持：图鉴 C-00030 锚正典位仍不在位〔anchors 20 止 C-00029〕"
    "+REACT 当日映射余量耗尽〔R314 判定·新热点窗=09-27 日报〕+ch.5 v3 稿未落〔storylines 三子域 09-26 零新写盘 python 实证〕+#57 10-07 窗+#21 周日件届日未到〔09-27=明日〕"
    "·自进池 open 真锚项全闭=idle-fast 收账〔并窗 6/6 满=batch commit R344-R349·窗重置 1/6〕）·state.ts/task 心跳面刷新"
)
OS_T = (
    "tick 349·R349（idle-fast·五静+探针绿——产线全 supply-gated 维持〔图鉴 C-00030 锚正典位轮首核仍不在位 anchors 20 止 C-00029·供给门只查 census/anchors/ 正典位"
    ...
    "②三探针全绿=board 0 FAIL 5 题 10 稿 5 in production/readiness 3 阻塞皆外部 CEO 面 0 发现/loop_health 0 FAIL 19 WARN 皆在案史实·tick348=done348 对账平"
    "③并窗 6/6 满=batch commit R344-R349（commit 注区间·窗重置 1/6）+P-61 导出步刷 export_ts）"
)
...
raw = load_raw(SP)
st = json.loads(raw)
assert st["tick"] == 348, "tick mismatch: %r" % st["tick"]
assert st["log"][-1].startswith("2026-09-26 07:23 R348"), "tail log is not R348"
st["tick"] = 349
...
assert chk["tick"] == 349 and chk["ts"] == ts_full and chk["task"] == BODY[:60]
...
print("CLOSE-DONE idle-fast window-6of6 batch-commit R344-R349")
