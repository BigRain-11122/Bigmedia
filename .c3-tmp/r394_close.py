# -*- coding: utf-8 -*-
"""R394 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 6/6 FULL (window R389-R394) -> batch commit R389-R394 this round per os-protocol S6."""
import json, io, os
...
FOCUS = (
    u"R395: #21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
...
line = (
    log_ts + " R394: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗 6/6 满=本窗 batch commit R389-R394）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R389~R393 idle-fast 并窗自记账预期态·"
    u"?? .c3-tmp/r389_close.py+r390_close.py+r391_close.py+r392_close.py+r393_close.py+r394_close.py "
    u"自产脚本件随本窗满批 commit〔窗 R389-R394〕·无 index.lock r359_check.py False 实证·HEAD=8d0ba25 未变）"
    ...
    u"〔12 log-order+8 heartbeat-gap〕·tick393=done393 对账平）；"
    ...
    u"窗满 6 轮触发=batch commit R389-R394（os-protocol §6 并窗律·commit 注区间·窗重置 1/6）·"
...
assert state["tick"] == 393, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R394:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R393:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 394 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R395:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R394 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
