# -*- coding: utf-8 -*-
# R364 idle-fast close (window round 3/6, NO commit this round):
...
entry = (
    u"2026-09-26 %s %s: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 3/6 不 commit）——"
    u"①无新令（orders 顶=O-20260925-1931-HQ-C·R283 已记账）"
    u"②backlog 顶行不可认领（#63 图鉴续件 C-00030 锚正典位轮首核=仍不在位〔anchors/C-00030.md+C-00031.md Test-Path False 双核实证·R363 r359_check.py 锚定续证·anchors 尾三=C-00027/C-00028/C-00029 止 C-00029〕supply-gated 维持"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R362-R364 idle-fast 并窗自记账预期态"
    u"·?? .c3-tmp/r362_close.py+r363_close.py+r363_probe.txt+r364_close.py 收账/证据件随窗满批 commit〔窗 R362-R367〕"
    u"·无 index.lock Test-Path False 实证·HEAD=14b7d22 未变）"
    u"④ch.5 v3 稿未落（storylines 递归扫描最新写盘=09-26 01:38 cards README〔R313 REACT-v2 收账件〕=novel/audio/comic 三子域零新写盘实证·ch.5 判定口径成立·bm-a 面）"
    u"·日报 2026-09-26 在案不重跑（mtime 00:02 未变）·W39 周审在案；"
    u"集团双锚静=ledger 行含 @ 四模式 21=锚零新转办（Select-String 计数 21 实证）"
    u"·decisions 非空行 33=锚〔尾=D-20260926-04〕零新行零动作；"
    u"三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0"
    u"/readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1〔renders 42/42 注账〕"
    u"/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick363=done363 对账平）；"
    ...
    u"——一行收账即出（idle-fast 并窗轮 3/6〔窗 R362-R367 满 6 收账〕·本轮不 commit·P-61 导出步照刷 export_ts）。"
    u"下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——全静即 idle-fast（4/6）。"
) % (now.strftime("%H:%M"), R)
...
assert old_tick == 363, "unexpected tick %d" % old_tick
...
assert data["tick"] == 364, "tick mismatch"
assert data["log"][-1].endswith(u"全静即 idle-fast（4/6）。"), "entry tail mismatch"
assert len(data["log"]) == 376, "log count unexpected: %d" % len(data["log"])
...
print("OK tick=364 ts=%s task_len=%d export_ts=%s" % (stamp_full, len(task), ts_iso))
