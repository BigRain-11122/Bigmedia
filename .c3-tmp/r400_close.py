# -*- coding: utf-8 -*-
"""R400 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 6/6 FULL (window R395-R400) -> batch commit R395-R400 per os-protocol S6, window reset."""
import json, io, os
...
FOCUS = (
    u"R401: #21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
    ...
    u"新令/集团转办/探针红出现即优先（异常或实活轮出现即提前收账）；全静即 idle-fast（新窗 1/6）"
)

line = (
    log_ts + " R400: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗 6/6 满=本窗 batch commit R395-R400）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R395~R399 idle-fast 并窗自记账预期态·"
    u"?? .c3-tmp/r395_close.py+r396_close.py+r397_close.py+r398_close.py+r399_close.py+r400_close.py "
    u"自产脚本件随本窗满批 commit〔窗 R395-R400〕·无 index.lock r359_check.py False 实证·HEAD=7655697 "
    u"未变·7655697 后零插队=无 bm-a 活跃写盘迹象）"
    ...
    u"（r359_check.py daily_brief_0926 True 实证）·W39 周审在案；"
    ...
    u"〔12 log-order+8 heartbeat-gap〕·tick399=done399 对账平）；"
    ...
    u"窗满 6 轮触发=batch commit R395-R400（os-protocol §6 并窗律·commit 注区间·窗重置 1/6）·"
...
assert state["tick"] == 399, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R400:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R399:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 400 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R401:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R400 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
