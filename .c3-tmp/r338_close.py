# -*- coding: utf-8 -*-
# R338 idle-fast close: state.json (tick/log/ts/task) + status-export.json (P-61)
...
entry = (
    "2026-09-26 " + ts_min + " R338: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 1/6 不 commit）——"
    ...
    "③树净零锁（HEAD=b7aba8b R332-R337 batch 收账后净态·git status 空输出·无 index.lock 实证）"
    ...
    "三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0〔首跑 src/os/ 路径笔误 exit 2=R328 同型·src/board_check.py 正路径复跑即绿〕/"
    "readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick337=done337 对账平）；"
...
assert '"tick": 337' in raw, 'tick anchor missing'
quoted = json.dumps(entry, ensure_ascii=False)
new = raw.replace(NL + ' ],', ',' + NL + '  ' + quoted + NL + ' ],', 1)
new = new.replace('"tick": 337', '"tick": 338', 1)
...
dept_t = ("OS 循环 R338（idle-fast·五静+探针绿——产线全 supply-gated 维持：图鉴 C-00030 锚正典位仍不在位〔anchors 止 C-00029〕"
          "+REACT 当日映射余量耗尽〔R314 判定·新热点窗=09-27 日报〕+ch.5 v3 稿未落〔bm-a〕+#57 10-07 窗+#21 周日件届日未到·"
          "自进池 open 真锚项全闭=idle-fast 一行收账〔并窗轮 1/6 不 commit〕）·state.ts/task 心跳面刷新")
for d in se['depts']:
    if d['n'] == '工程技术部':
        d['t'] = dept_t
outs_t = ("tick 338·R338（idle-fast·五静+探针绿——产线全 supply-gated 维持〔图鉴 C-00030 锚正典位轮首核仍不在位 anchors 止 C-00029·"
          ...
          "+集团双锚静〔ledger 行含 @ 四模式 21=锚零新转办·decisions python 非空行 33=锚零新行〕+树净零锁 HEAD=b7aba8b"
          "②三探针全绿=board 0 FAIL 5 题 10 稿 5 in production/readiness 3 阻塞皆外部 CEO 面 0 发现/"
          "loop_health 0 FAIL 19 WARN 皆在案史实·tick337=done337 对账平③并窗轮 1/6 不 commit〔os-protocol §6 并窗律〕"
          "+P-61 导出步刷 export_ts）")
