# -*- coding: utf-8 -*-
# r439 five-check (r438_check.py pattern, HEAD anchor 204b012=R433-R438 batch close commit, window R439-R444 uncommitted; this round = window 1/6, no commit)
...
# 0b) bm-a insert commits after 204b012 (R433-R438 batch close commit anchor)
r = subprocess.run(["git", "log", "--oneline", "204b012..HEAD"], capture_output=True, text=True, encoding="utf-8", errors="replace")
inter = [x for x in (r.stdout or "").splitlines() if x.strip()]
log("inserts_after_204b012=%d" % len(inter))
if inter:
    for x in inter[:5]:
        log("insert=" + x[:100])
...
out_path = os.path.join(repo, ".c3-tmp", "r439_probe.txt")
...
with io.open(os.path.join(repo, ".c3-tmp", "r439_check.txt"), "w", encoding="utf-8") as f:
    f.write("\n".join(L) + "\n")
print("check_file=.c3-tmp/r439_check.txt")
