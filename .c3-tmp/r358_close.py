# -*- coding: utf-8 -*-
# R358 idle-fast close: state.json append + tick/ts/task refresh + status-export export_ts refresh
...
R = "R358"
WINDOW = "idle-fast 并窗轮 3/6（本轮不 commit·窗 R356-R361 满 6 收账）"
...
entry = (
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R356/R357 idle-fast 并窗自记账预期态·?? .c3-tmp/r356~r358 自产脚本件随窗满批 commit·无 index.lock python os.path.exists False 实证·HEAD=5a9ee5c）"
    ...
    u"/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick357=done357 对账平）；"
...
assert "R358: idle-fast" in entry
...
assert old_tick == 357, "unexpected tick %d" % old_tick
...
assert data["tick"] == 358, "tick mismatch"
assert data["log"][-1].endswith(u"batch commit R356-R361）。"), "entry tail mismatch"
assert len(data["log"]) == 370, "log count unexpected: %d" % len(data["log"])
...
print("OK tick=358 ts=%s task_len=%d export_ts=%s" % (stamp_full, len(task), ts_iso))
