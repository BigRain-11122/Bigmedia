# -*- coding: utf-8 -*-
"""R385 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 3/6 (window R383-R388) -> no commit this round (os-protocol S6 batching)."""
import json, io, os
...
FOCUS = (
    u"R386: #21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
...
line = (
    log_ts + " R385: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 3/6 不 commit）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R383/R384 idle-fast 并窗自记账预期态·"
    u"?? .c3-tmp/r383_close.py+r385_close.py 自产脚本件随窗满批 commit〔窗 R383-R388〕·"
    u"无 index.lock os.path.exists False 实证·HEAD=d784e51 未变）"
    ...
    u"一行收账即出（idle-fast 并窗轮 3/6〔窗 R383-R388 满 6 收账〕·本轮不 commit·P-61 导出步照刷 "
...
assert state["tick"] == 384, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R385:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R384:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 385 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R386:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R385 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
