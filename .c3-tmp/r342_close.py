# R342 idle-fast close: state.json (tick 341->342, log append, ts/task refresh)
...
log_prefix = now.strftime("%Y-%m-%d %H:%M") + " R342: "
...
BODY = (
    ...
    "③树态=仅自产预期件（M state.json+M status-export.json=R338-R341 idle-fast 并窗自记账预期态·os-protocol §6·?? .c3-tmp/r338~r342 七自产脚本件随窗满批 commit·无 index.lock Test-Path False 实证·HEAD=b7aba8b）"
    "④ch.5 v3 稿未落（storylines python 扫描 09-26 零新写盘实证：novel 止 SC-001-04-v3+SC-001-05-v1·audio 止 sc001-05-v1-tmp·comic 止 SC-002-02-v1·bm-a 面）·"
    ...
    "三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0/readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick341=done341 对账平）；"
    ...
    "下轮快速路径首查：R343=并窗 6/6 窗满收账 commit（区间消息注明 R338-R343 idle-fast batch）+"
...
ENG_T = (
    "OS 循环 R342（idle-fast·五静+探针绿——产线全 supply-gated 维持：图鉴 C-00030 锚正典位仍不在位〔anchors 20 止 C-00029〕"
    "+REACT 当日映射余量耗尽〔R314 判定·新热点窗=09-27 日报〕+ch.5 v3 稿未落〔bm-a 面 09-26 零新写盘〕"
    "+#57 10-07 窗+#21 周日件届日未到·自进池 open 真锚项全闭=idle-fast 一行收账〔并窗轮 5/6 不 commit〕）·state.ts/task 心跳面刷新"
)
OS_T = (
    "tick 342·R342（idle-fast·五静+探针绿——产线全 supply-gated 维持〔图鉴 C-00030 锚正典位轮首核仍不在位 anchors 20 止 C-00029·供给门只查 census/anchors/ 正典位"
    ...
    "+树态=仅自产预期件〔M state.json+M status-export=R338-R341 并窗自记账预期态〕"
    "②三探针全绿=board 0 FAIL 5 题 10 稿 5 in production/readiness 3 阻塞皆外部 CEO 面 0 发现/loop_health 0 FAIL 19 WARN 皆在案史实·tick341=done341 对账平"
    "③并窗轮 5/6 不 commit〔os-protocol §6 并窗律〕+P-61 导出步刷 export_ts）"
)
...
raw = load_raw(SP)
st = json.loads(raw)
assert st["tick"] == 341, "tick mismatch: %r" % st["tick"]
assert st["log"][-1].startswith("2026-09-26 06:13 R341"), "tail log is not R341"
st["tick"] = 342
...
assert chk["tick"] == 342 and chk["ts"] == ts_full and chk["task"] == BODY[:60]
...
print("CLOSE-DONE no-commit-window-5of6")
