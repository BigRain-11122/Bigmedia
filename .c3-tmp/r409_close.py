# -*- coding: utf-8 -*-
# R409 idle-fast close (window 3/6 of R407-R412, no commit): state.json tick/log/ts/task + status-export export_ts (P-61)
...
logline = (
    f"{stamp} R409: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 3/6 不 commit）——"
    ...
    "③树态=仅自产预期件（M state.json+M status-export.json=R408 idle-fast 并窗自记账预期态·"
    "?? .c3-tmp/r407_close.py+r408_close.py+r409_close.py 自产脚本件随窗满批 commit〔窗 R407-R412〕·无 index.lock r359_check.py False 实证·"
    ...
    "loop_health 0 FAIL 20 WARN 皆在案史实〔12 log-order+8 heartbeat-gap〕·tick408=done408 对账平）；"
    ...
    "一行收账即出（idle-fast 并窗轮 3/6〔窗 R407-R412 满 6 收账〕·本轮不 commit·P-61 导出步照刷 export_ts）。"
...
with io.open(SP, encoding='utf-8') as f:
    st = json.load(f)
assert st['tick'] == 408, st['tick']
st['tick'] = 409
old_focus = st['focus']
assert old_focus.startswith('R409:'), old_focus[:40]
new_focus = old_focus.replace('R409:', 'R410:', 1)
new_focus = new_focus.replace('全静即 idle-fast（3/6）', '全静即 idle-fast（4/6）', 1)
