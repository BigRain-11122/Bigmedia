# -*- coding: utf-8 -*-
"""R393 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 5/6 (window R389-R394) -> no commit this round per os-protocol S6."""
import json, io, os
...
FOCUS = (
    u"R394: 并窗 6/6 窗满=batch commit R389-R394（os-protocol §6 并窗律·区间消息注明 idle-fast batch）+"
    u"#21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
    ...
    u"新令/集团转办/探针红出现即优先（异常或实活轮出现即提前收账）；全静即 idle-fast 窗满收账"
)

line = (
    log_ts + " R393: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 5/6 不 commit）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R389~R392 idle-fast 并窗自记账预期态·"
    u"?? .c3-tmp/r389_close.py+r390_close.py+r391_close.py+r392_close.py 自产脚本件随窗满批 commit"
    u"〔窗 R389-R394〕·无 index.lock r359_check.py False 实证·HEAD=8d0ba25 未变·8d0ba25 后零插队="
    u"无 bm-a 活跃写盘迹象）"
    ...
    u"（os.path.exists True·r359_check.py 实证）·W39 周审在案；"
    ...
    u"〔12 log-order+8 heartbeat-gap〕·tick392=done392 对账平）；"
    ...
    u"一行收账即出（idle-fast 并窗轮 5/6〔窗 R389-R394 满 6 收账〕·本轮不 commit·"
    u"P-61 导出步照刷 export_ts）。下轮=R394 并窗 6/6 窗满收账 commit（区间消息注明 R389-R394 idle-fast "
    u"batch·窗重置 1/6）+#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗"
    u"届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——异常或实活轮出现即提前收账。"
)

with io.open(STATE, "r", encoding="utf-8") as f:
    state = json.load(f)

assert state["tick"] == 392, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R393:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R392:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 393 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R394:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R393 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
