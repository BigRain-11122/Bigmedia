# -*- coding: utf-8 -*-
"""R386 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 4/6 (window R383-R388) -> no commit this round (os-protocol S6 batching)."""
import json, io, os
...
FOCUS = (
    u"R387: #21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
...
line = (
    log_ts + " R386: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 4/6 不 commit）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R383~R385 idle-fast 并窗自记账预期态·"
    u"?? .c3-tmp/r383_close.py+r385_close.py+r386_close.py 自产脚本件随窗满批 commit〔窗 R383-R388〕·"
    u"无 index.lock r359_check.py False 实证·HEAD=d784e51 未变）"
    ...
    u"一行收账即出（idle-fast 并窗轮 4/6〔窗 R383-R388 满 6 收账〕·本轮不 commit·P-61 导出步照刷 "
    u"export_ts）。下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/"
    u"REACT 09-27 日报热点窗届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——"
    u"全静即 idle-fast（5/6）。"
)
...
assert state["tick"] == 385, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R386:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R385:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 386 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R387:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R386 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
