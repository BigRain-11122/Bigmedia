# R355 window-full close: state.json (tick/log/ts/task) + status-export.json (P-61)
...
prefix = "%s R355: " % stamp
body = (
    "idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗 6/6 满=本窗 batch commit R350-R355）——"
    ...
    "#59 REACT 当日映射余量耗尽 R314 判定同日维持〔日报 mtime 09-26 00:02 未变=R313/R314 判据输入零变化·新热点窗=09-27 日报〕·#57 替代率首报 10-07 窗挂账·#21 周日立法件届日=明日 09-27 未到·"
    "自进池 open 真锚项全闭〔A 池全 done+B 池 B1-B4 done·B5 账号期门控+C 池 C1-C3 done·C4 常态位 blocked on 进链件〕）"
    "③树态=仅自产预期件（M state.json+M status-export.json=R350~R354 idle-fast 并窗自记账预期态·?? .c3-tmp/r350~r355 自产脚本件 14 件随本窗满批 commit·无 index.lock python os.path.exists False 实证·HEAD=180af8a 未变）"
    ...
    "三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0/readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1〔renders 42/42 注账〕/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick354=done354 对账平）；"
    ...
    "窗满 6 轮触发=batch commit R350-R355（os-protocol §6 并窗律·commit 注区间·窗重置 1/6）·P-61 导出步照刷 export_ts。"
...
with open(SP, encoding="utf-8") as f:
    st = json.load(f)
assert st["tick"] == 354, "tick anchor mismatch: %s" % st["tick"]
assert "R354" in st["log"][-1], "last log anchor mismatch"
st["tick"] = 355
...
with open(XP, encoding="utf-8") as f:
    ex = json.load(f)
assert ex["export_ts"].startswith("2026-09-26T08:24"), "export_ts anchor mismatch: %s" % ex["export_ts"]
ex["export_ts"] = now.strftime("%Y-%m-%dT%H:%M:%S+08:00")
eng_t = (
    "OS 循环 R355（idle-fast·五静+探针绿——产线全 supply-gated 维持：图鉴 C-00030 锚正典位仍不在位〔anchors 20 止 C-00029〕"
    "+REACT 当日映射余量耗尽〔R314 判定·日报 mtime 未变·新热点窗=09-27 日报〕+ch.5 v3 稿未落〔storylines 三子域 09-26 零新写盘 python 实证〕"
    "+#57 10-07 窗+#21 周日件届日明日 09-27·自进池 open 真锚项全闭=idle-fast 收账〔并窗 6/6 满=本窗 batch commit R350-R355·窗重置 1/6〕）·state.ts/task 心跳面刷新"
)
found = False
for d in ex["depts"]:
    if d["n"] == "工程技术部":
...
os_out = (
    "tick 355·R355（idle-fast·五静+探针绿——产线全 supply-gated 维持〔图鉴 C-00030 锚正典位轮首核仍不在位 anchors 20 止 C-00029·供给门只查 census/anchors/ 正典位"
    ...
    "②三探针全绿=board 0 FAIL 5 题 10 稿 5 in production/readiness 3 阻塞皆外部 CEO 面 0 发现〔renders 42/42 注账〕/loop_health 0 FAIL 19 WARN 皆在案史实·tick354=done354 对账平"
    "③并窗 6/6 满=batch commit R350-R355（os-protocol §6 并窗律·commit 注区间·窗重置 1/6）+P-61 导出步刷 export_ts）"
)
...
for pth in (SP, XP):
    with open(pth, encoding="utf-8") as f:
        json.load(f)
print("OK tick=355 ts=%s task_len=%d export_ts=%s" % (ts_full, len(body[:60]), ex["export_ts"]))
