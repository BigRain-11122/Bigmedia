# -*- coding: utf-8 -*-
"""R387 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 5/6 (window R383-R388) -> no commit this round (os-protocol S6 batching)."""
import json, io, os
...
FOCUS = (
    u"R388: #21 周日立法件 09-27 届日即领（周日周轮立法流程）→"
...
line = (
    log_ts + " R387: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 5/6 不 commit）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R383~R386 idle-fast 并窗自记账预期态·"
    u"?? .c3-tmp/r383_close.py+r385_close.py+r386_close.py+r387_close.py 自产脚本件随窗满批 commit"
    u"〔窗 R383-R388〕·无 index.lock r359_check.py False 实证·HEAD=d784e51 未变）"
    ...
    u"（os.path.exists True）·W39 周审在案；"
    u"集团双锚静=ledger @ 五模式 23=锚零新转办（尾=P-20260926-03 R377 已收讫）·decisions python "
    u"非空行 40=锚〔尾=D-20260926-11 R377 已定谳〕零新行零动作；"
    ...
    u"一行收账即出（idle-fast 并窗轮 5/6〔窗 R383-R388 满 6 收账〕·本轮不 commit·P-61 导出步照刷 "
    u"export_ts）。下轮快速路径首查：R388=并窗 6/6 窗满收账 commit（区间消息注明 R383-R388 idle-fast "
    u"batch）+#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/"
    u"图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——异常或实活轮出现即提前收账。"
)

with io.open(STATE, "r", encoding="utf-8") as f:
    state = json.load(f)

assert state["tick"] == 386, "tick drift: %s" % state["tick"]
assert line.startswith(log_ts + " R387:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R386:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 387 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
assert chk["focus"].startswith(u"R388:"), "focus refresh failed"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R387 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
