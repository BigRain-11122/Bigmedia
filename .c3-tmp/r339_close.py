# -*- coding: utf-8 -*-
# R339 idle-fast close: state.json (tick/log/ts/task) + status-export.json (P-61)
...
entry = (
    "2026-09-26 " + ts_min + " R339: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 2/6 不 commit）——"
    "①无新令（orders 顶=O-20260925-1931-HQ-C·R283 已记账）"
    "②backlog 顶行不可认领（#63 图鉴续件 C-00030 锚正典位轮首核=仍不在位〔python os.path.exists False+C-00031 同核 False 实证·anchors 止 C-00029〕supply-gated 维持·"
    ...
    "③树态=仅自产预期件（M state.json+M status-export.json=R338 idle-fast 并窗自记账预期态·os-protocol §6·?? .c3-tmp/r338_close.py=自产收账脚本件随窗批 commit·无 index.lock Test-Path False 实证·HEAD=b7aba8b）"
    "④ch.5 v3 稿未落（novel 目录实证止 SC-001-04-v3+SC-001-05-v1·ch.6 未现·bm-a 面 09-26 零新写盘）·日报 2026-09-26 在案不重跑（Test-Path True）·W39 周审在案；"
    "集团双锚静=ledger 行含 @ 四模式 21=锚零新转办（python 计数 21 实证）·decisions python 非空行 33=锚〔尾=D-20260926-04〕零新行零动作；"
    "三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0/readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick338=done338 对账平）；"
...
assert '"tick": 338' in raw, 'tick anchor missing'
quoted = json.dumps(entry, ensure_ascii=False)
new = raw.replace(NL + ' ],', ',' + NL + '  ' + quoted + NL + ' ],', 1)
new = new.replace('"tick": 338', '"tick": 339', 1)
...
dept_t = ("OS 循环 R339（idle-fast·五静+探针绿——产线全 supply-gated 维持：图鉴 C-00030 锚正典位仍不在位〔anchors 止 C-00029〕"
          "+REACT 当日映射余量耗尽〔R314 判定·新热点窗=09-27 日报〕+ch.5 v3 稿未落〔bm-a〕+#57 10-07 窗+#21 周日件届日未到·"
          "自进池 open 真锚项全闭=idle-fast 一行收账〔并窗轮 2/6 不 commit〕）·state.ts/task 心跳面刷新")
for d in se['depts']:
    if d['n'] == '工程技术部':
        d['t'] = dept_t
outs_t = ("tick 339·R339（idle-fast·五静+探针绿——产线全 supply-gated 维持〔图鉴 C-00030 锚正典位轮首核仍不在位 anchors 止 C-00029·"
          ...
          "+集团双锚静〔ledger 行含 @ 四模式 21=锚零新转办·decisions python 非空行 33=锚零新行〕+树态=仅自产预期件〔M state.json+M status-export=R338 并窗自记账预期态〕"
          "②三探针全绿=board 0 FAIL 5 题 10 稿 5 in production/readiness 3 阻塞皆外部 CEO 面 0 发现/"
          "loop_health 0 FAIL 19 WARN 皆在案史实·tick338=done338 对账平③并窗轮 2/6 不 commit〔os-protocol §6 并窗律〕"
          "+P-61 导出步刷 export_ts）")
