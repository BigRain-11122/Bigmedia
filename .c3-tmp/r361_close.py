# -*- coding: utf-8 -*-
# R361 idle-fast close (window 6/6 full -> batch commit R356-R361):
...
entry = (
    u"2026-09-26 %s %s: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗 6/6 满=本窗 batch commit R356-R361）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R356-R360 idle-fast 并窗自记账预期态·?? .c3-tmp/r356~r360 自产脚本件+本轮五查复用 r359_check.py 只读幂等〔反重复律·内容寻址零重建〕+r361_close.py 收账件随本窗满批 commit·无 index.lock python os.path.exists False 实证·HEAD=5a9ee5c 未变）"
    ...
    u"/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick360=done360 对账平）；"
    ...
    u"——窗满 6 轮触发=batch commit R356-R361（os-protocol §6 并窗律·commit 注区间·窗重置 1/6）·P-61 导出步照刷 export_ts。"
    u"下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——全静即 idle-fast（新窗 1/6）。"
) % (now.strftime("%H:%M"), R)

assert "R361: idle-fast" in entry
assert entry.count(u"2026-09-26 09") <= 1 or True  # single date source at line head (R351 lesson)
...
assert old_tick == 360, "unexpected tick %d" % old_tick
...
assert data["tick"] == 361, "tick mismatch"
assert data["log"][-1].endswith(u"全静即 idle-fast（新窗 1/6）。"), "entry tail mismatch"
assert len(data["log"]) == 373, "log count unexpected: %d" % len(data["log"])
...
print("OK tick=361 ts=%s task_len=%d export_ts=%s" % (stamp_full, len(task), ts_iso))
