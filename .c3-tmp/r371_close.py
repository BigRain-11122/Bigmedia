# -*- coding: utf-8 -*-
"""R371 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 4/6 -> no commit this round (os-protocol S6 batch rule, window R368-R373)."""
import json, io, os
...
line = (
    log_ts + " R371: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 4/6 不 commit）——"
    "①无新令（orders 顶=O-20260925-1931-HQ-C·R283 已记账）"
    "②backlog 顶行不可认领（#63 图鉴续件 C-00030 锚正典位轮首核=仍不在位〔Test-Path False+r359_check.py "
    ...
    "③树态=仅自产预期件（M state.json+M status-export.json=R368-R370 idle-fast 并窗自记账预期态·"
    "?? .c3-tmp/r368_close.py+r368_taskfix.py+r369_close.py+r370_* 探针件+r370_close.py+r371_tail.txt+"
    "r371_close.py 自产脚本/证据件随窗满批 commit〔窗 R368-R373〕·无 index.lock r359_check.py False 实证·"
    "HEAD=9e79ca0 未变）"
    ...
    "三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0〔正确路径 src/board_check.py 直调·零操作红〕/"
    "readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1〔renders 42/42 注账〕/"
    "loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick370=done370 对账平）；"
    ...
    "一行收账即出（idle-fast 并窗轮 4/6〔窗 R368-R373 满 6 收账〕·本轮不 commit·P-61 导出步照刷 export_ts）。"
    "下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/"
    "图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——全静即 idle-fast（5/6）。"
)

with io.open(STATE, "r", encoding="utf-8") as f:
    state = json.load(f)

assert line.startswith(log_ts + " R371:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R370:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 371 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] == line, "log tail mismatch"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R371 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
