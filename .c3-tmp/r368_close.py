# -*- coding: utf-8 -*-
"""R368 idle-fast close-out: state.json log line + tick/ts/task + status-export export_ts.
Window round 1/6 -> no commit this round (os-protocol S6 batch rule)."""
...
line = (
    log_ts + " R368: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 1/6 不 commit）——"
    ...
    "③树净零锁（HEAD=9e79ca0 R362-R367 batch 收账后净态·git status 空输出·无 index.lock python "
    "os.path.exists False 实证·本轮自产 M state.json+M status-export.json+?? .c3-tmp/r368_close.py="
    "并窗自记账预期态非 bm-a 迹象〔r368_close.py 随窗满批 commit〕）"
    ...
    "〔12 log-order+7 heartbeat-gap〕·tick367=done367 对账平）；"
    ...
    "一行收账即出（idle-fast 并窗轮 1/6〔窗 R368-R373 满 6 收账〕·本轮不 commit·P-61 导出步照刷 export_ts）。"
    "下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/"
    "图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——全静即 idle-fast（2/6）。"
)
...
assert line.startswith(log_ts + " R368:"), "log line stamp mismatch"
prev_ts = state["log"][-1].split(" R367:")[0]
...
with io.open(STATE, "r", encoding="utf-8") as f:
    chk = json.load(f)
assert chk["tick"] == 368 and chk["ts"] == ts_full, "state re-check failed"
assert chk["log"][-1] is line or chk["log"][-1] == line, "log tail mismatch"
with io.open(EXPORT, "r", encoding="utf-8") as f:
    echk = json.load(f)
assert echk["export_ts"] == export_ts, "export re-check failed"
print("R368 close ok: tick=%s ts=%s export_ts=%s log_lines=%d" % (chk["tick"], chk["ts"], echk["export_ts"], len(chk["log"])))
