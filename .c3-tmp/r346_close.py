# R346 idle-fast close: state.json (tick 345->346, log append, ts/task refresh)
# + P-61 export step (status-export.json export_ts/depts/outs/results derived refresh)
# Window 3/6 -> NO commit this round (os-protocol S6 batching rule).
# Pattern reuse: r344_close.py (R322 lesson: state.json multi-line edits via one-shot python).
...
log_prefix = now.strftime("%Y-%m-%d %H:%M") + " R346: "
...
BODY = (
    ...
    "③树态=仅自产预期件（M state.json+M status-export.json=R344/R345 idle-fast 并窗自记账预期态·?? .c3-tmp/r344_close.py+r345_check.py=自产脚本件随窗满批 commit·"
    "无 index.lock Test-Path False 实证·HEAD=b7e43f4）"
    "④ch.5 v3 稿未落（novel python os.listdir 实证 SC-001-05 止 v1 三件口径·bm-a 面 09-26 零新写盘）·"
    ...
    "三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0/readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick345=done345 对账平）；"
...
ENG_T = (
    "OS 循环 R346（idle-fast·五静+探针绿——产线全 supply-gated 维持：图鉴 C-00030 锚正典位仍不在位〔anchors 20 止 C-00029〕"
    "+REACT 当日映射余量耗尽〔R314 判定·新热点窗=09-27 日报〕+ch.5 v3 稿未落〔bm-a 面〕+#57 10-07 窗+#21 周日件届日未到〔09-27=明日〕"
    "·自进池 open 真锚项全闭=idle-fast 一行收账〔并窗轮 3/6·本轮不 commit〕）·state.ts/task 心跳面刷新"
)
OS_T = (
    "tick 346·R346（idle-fast·五静+探针绿——产线全 supply-gated 维持〔图鉴 C-00030 锚正典位轮首核仍不在位 anchors 20 止 C-00029·供给门只查 census/anchors/ 正典位"
    ...
    "②三探针全绿=board 0 FAIL 5 题 10 稿 5 in production/readiness 3 阻塞皆外部 CEO 面 0 发现/loop_health 0 FAIL 19 WARN 皆在案史实·tick345=done345 对账平"
    "③并窗轮 3/6 不 commit+P-61 导出步刷 export_ts）"
)
...
raw = load_raw(SP)
st = json.loads(raw)
assert st["tick"] == 345, "tick mismatch: %r" % st["tick"]
assert st["log"][-1].startswith("2026-09-26 06:55 R345"), "tail log is not R345"
st["tick"] = 346
...
assert chk["tick"] == 346 and chk["ts"] == ts_full and chk["task"] == BODY[:60]
...
print("CLOSE-DONE idle-fast window-3of6 no-commit")
