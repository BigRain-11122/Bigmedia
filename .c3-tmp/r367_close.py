# -*- coding: utf-8 -*-
# R367 idle-fast close (window round 6/6 FULL -> batch commit R362-R367):
...
entry = (
    u"2026-09-26 %s %s: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗 6/6 满=本窗 batch commit R362-R367）——"
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R362-R366 idle-fast 并窗自记账预期态"
    u"·?? .c3-tmp/r362_close.py+r363_close.py+r363_probe.txt+r364_close.py+r365_close.py+r366_close.py+r367_probes.py+r367_probe.txt+r367_close.py 收账/证据件随本窗满批 commit〔窗 R362-R367〕"
    u"·无 index.lock python os.path.exists False 实证·HEAD=14b7d22 未变）"
    u"④ch.5 v3 稿未落（storylines python walk 扫描 novel/audio/comic 三子域 09-26 零新写盘实证〔r359_check.py 读数 0/0/0〕=ch.5 判定口径成立·bm-a 面）"
    ...
    u"/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick366=done366 对账平）；"
    ...
    u"——窗满 6 轮触发=batch commit R362-R367（os-protocol §6 并窗律·commit 注区间·窗重置 1/6）·P-61 导出步照刷 export_ts。"
    u"下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——全静即 idle-fast（新窗 1/6）。"
) % (now.strftime("%H:%M"), R)
...
assert old_tick == 366, "unexpected tick %d" % old_tick
...
assert data["tick"] == 367, "tick mismatch"
assert data["log"][-1].endswith(u"全静即 idle-fast（新窗 1/6）。"), "entry tail mismatch"
assert len(data["log"]) == 379, "log count unexpected: %d" % len(data["log"])
...
print("OK tick=367 ts=%s task_len=%d export_ts=%s" % (stamp_full, len(task), ts_iso))
