# -*- coding: utf-8 -*-
# R357 idle-fast close: state.json append + tick/ts/task refresh + status-export export_ts refresh
...
stamp_hm = now.strftime("%Y-%m-%d %H:%M")
stamp_full = now.strftime("%Y-%m-%d %H:%M:%S")
R = "R357"
WINDOW = "idle-fast 并窗轮 2/6（本轮不 commit·窗 R356-R361 满 6 收账）"

entry = (
    u"2026-09-26 %s %s: idle-fast（快速路径·五静+探针绿·不进开轮四步·并窗轮 2/6 不 commit）——"
    u"①无新令（orders 顶=O-20260925-1931-HQ-C·R283 已记账）"
    u"②backlog 顶行不可认领（#63 图鉴续件 C-00030 锚正典位轮首核=仍不在位〔python os.path.exists False+C-00031 同核 False 实证·anchors 20 止 C-00029 尾二=C-00028/C-00029〕supply-gated 维持"
    u"·#59 REACT 当日映射余量耗尽 R314 判定同日维持〔日报 mtime 09-26 00:02 未变=R313/R314 判据输入零变化·新热点窗=09-27 日报〕"
    u"·#57 替代率首报 10-07 窗挂账·#21 周日立法件届日=明日 09-27 未到〔明日周日届日=下轮首查〕"
    u"·自进池 open 真锚项全闭〔A 池全 done+B 池 B1-B4 done·B5 账号期门控+C 池 C1-C3 done·C4 常态位 blocked on 进链件〕）"
    u"③树态=仅自产预期件（M state.json+M status-export.json=R356 idle-fast 并窗自记账预期态·?? .c3-tmp/r356~r357 自产脚本件随窗满批 commit·无 index.lock python 实证·HEAD=5a9ee5c）"
    u"④ch.5 v3 稿未落（storylines python walk 扫描 novel/audio/comic 三子域 09-26 零新写盘实证〔novel SC-001-05 止 v1·ch.6 未现〕=ch.5 判定口径成立·bm-a 面）"
    u"·日报 2026-09-26 在案不重跑（os.path.exists True）·W39 周审在案；"
    u"集团双锚静=ledger 行含 @ 四模式 21=锚零新转办（python 计数 21 实证）·decisions python 非空行 33=锚〔尾=D-20260926-04·mtime 09-26 00:15 未变〕零新行零动作；"
    u"三探针全绿（board 0 FAIL 5 题 10 稿 5 in production exit 0/readiness 3 阻塞皆外部 CEO 物理件+决策 0 发现=阻塞≠失败口径 exit 1〔renders 42/42 注账〕"
    u"/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick356=done356 对账平）；"
    ...
    u"下轮快速路径首查：#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——全静即 idle-fast（6/6 窗满→batch commit R356-R361）。"
) % (now.strftime("%H:%M"), R, WINDOW)

assert "R357: idle-fast" in entry
...
assert old_tick == 356, "unexpected tick %d" % old_tick
...
assert data["tick"] == 357, "tick mismatch"
assert data["log"][-1].endswith("batch commit R356-R361）。"), "entry tail mismatch"
assert len(data["log"]) == 369, "log count unexpected: %d" % len(data["log"])
...
print("OK tick=357 ts=%s task_len=%d export_ts=%s" % (stamp_full, len(task), ts_iso))
