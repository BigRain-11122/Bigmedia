# -*- coding: utf-8 -*-
# r425 fast-path five-check (r424_check.py pattern) + three probes
...
# 0b) bm-a insert commits after c7c6bef (R419-R424 window batch anchor)
r = subprocess.run(["git", "log", "--oneline", "c7c6bef..HEAD"], capture_output=True, text=True, encoding="utf-8", errors="replace")
inter = [x for x in (r.stdout or "").splitlines() if x.strip()]
log("inserts_after_c7c6bef=%d" % len(inter))
if inter:
    for x in inter[:5]:
        log("insert=" + x[:100])
...
out_path = os.path.join(repo, ".c3-tmp", "r425_probe.txt")
...
with io.open(os.path.join(repo, ".c3-tmp", "r425_check.txt"), "w", encoding="utf-8") as f:
    f.write("\n".join(L) + "\n")
print("check_file=.c3-tmp/r425_check.txt")
