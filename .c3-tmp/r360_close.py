# -*- coding: utf-8 -*-
# R360 idle-fast close: state.json append + tick/ts/task refresh + status-export export_ts refresh
...
R = "R360"
WINDOW = u"idle-fast 并窗轮 5/6（本轮不 commit·窗 R356-R361 满 6 收账）"
...
entry = (
    ...
    u"③树态=仅自产预期件（M state.json+M status-export.json=R356-R359 idle-fast 并窗自记账预期态·?? .c3-tmp/r356~r359 自产脚本件+本轮五查复用 r359_check.py 只读幂等〔反重复律·内容寻址零重建〕·r360_close.py 收账件随窗满批 commit·无 index.lock python os.path.exists False 实证·HEAD=5a9ee5c 未变）"
    ...
    u"/loop_health 0 FAIL 19 WARN 皆在案史实〔12 log-order+7 heartbeat-gap〕·tick359=done359 对账平）；"
    ...
    u"下轮快速路径首查：R361=并窗 6/6 窗满收账 commit（区间消息注明 R356-R361 idle-fast batch）+#21 周日立法件（09-27 周日届日即领·周日周轮立法流程）/REACT 09-27 日报热点窗届日即领/图鉴 C-00030 锚/ch.5 v3 稿落迹象/新令/集团转办——异常或实活轮出现即提前收账。"
) % (now.strftime("%H:%M"), R, WINDOW)

assert "R360: idle-fast" in entry
...
assert old_tick == 359, "unexpected tick %d" % old_tick
...
assert data["tick"] == 360, "tick mismatch"
assert data["log"][-1].endswith(u"异常或实活轮出现即提前收账。"), "entry tail mismatch"
assert len(data["log"]) == 372, "log count unexpected: %d" % len(data["log"])
...
print("OK tick=360 ts=%s task_len=%d export_ts=%s" % (stamp_full, len(task), ts_iso))
