# -*- coding: utf-8 -*-
"""S1 gate call for BS-005e Tuanjie-engine enhanced piece (pillar-4 debut,
N6-mapped 6th piece, script chain start, R203).

Same channel as .bs001-dy-tmp/s1_call.py (R201/R202 proven): reuses
call_expert registry/build_prompt/call_model/verdict-archive/ledger-row
verbatim, 1500s timeout. Run detached (Start-Process) so the 5-min
no-output guard does not cancel it. Result -> .bs005e-tmp/s1-result.json.
Material = s1-review-material.md (12 beats v1, machine-narrator
system-log voice; S1 v1.5 violation-deduction gate).
ASCII source; Chinese stays in data files.
"""
import json
import subprocess
import sys
...
MATERIAL = sys.argv[1] if len(sys.argv) > 1 else str(
    REPO / "data" / "sources" / "bs005e" / "s1-review-material.md")
RESULT = REPO / ".bs005e-tmp" / "s1-result.json"
...
def main():
    ...
    if code == 0 and out.strip():
        ...
        note = "full text=expert-verdicts/%s | %s (1500s wrapper: S1 v1.5 gate, BS-005e Tuanjie-engine enhanced beats v1)" % (vname, first_line)
