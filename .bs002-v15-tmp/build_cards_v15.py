# -*- coding: utf-8 -*-
# R450: build cards-v15-matched.json for the F-002 two-law remake
# inherit: v1-matched lines+visuals (card anchors = v5 originals, R449
# card-column zero-move), timings from the v15 TTS baseline (v15 audio clock).
import json, io

base = json.load(io.open('.bs002-v15-tmp/cards.json', encoding='utf-8'))
v1 = json.load(io.open('data/sources/bs002/cards-v1-matched.json',
                       encoding='utf-8'))
assert len(base['cards']) == len(v1['cards']) == 12

# line diffs expected zero: v15 beats kept the card anchor column at v5
# originals (R449), the v1-matched file carries those same anchor lines
diffs = 0
...
out['meta']['beats'] = 'data\\sources\\bs002\\voiceover-v15.beats.txt'
out['meta']['order'] = ('P-20260926-11 #71 remake leg-3 F-002: two-law '
                        'remake of the v2b shipinhao piece')

with io.open('data/sources/bs002/cards-v15-matched.json', 'w',
             encoding='utf-8') as f:
