# R249 fast-path scan: ledger @ lines, group decisions count, novel/comic progress,
# window enumeration (Biggame master-control window check), three probes.
# Output written to UTF-8 files (encoding law: no console-trust for CJK).
import ctypes
import datetime
import glob
import io
...
# --- 4. window enumeration (Biggame master-control window check) ---
...
buf.write("window_count=%d biggame_hits=%d\n" % (len(titles), len(biggame)))
for t in titles:
    buf.write("  W| " + t + "\n")

with open(os.path.join(TMP, "r249-scan.txt"), "w", encoding="utf-8") as f:
...
for name, args in probes:
    try:
        ...
        with open(os.path.join(TMP, "probe-r249-%s.txt" % name), "w", encoding="utf-8") as f:
            f.write("exit=%d\n" % r.returncode)
            f.write(text)
            if err:
                f.write("\n--- stderr ---\n")
...
with open(os.path.join(TMP, "r249-scan.txt"), "a", encoding="utf-8") as f:
    f.write("probes: " + "; ".join(summary) + "\n")

print("OK now=%s ledger=%d biggame=%d %s" % (
    now.strftime("%H:%M:%S"),
    len(hits), len(biggame), "; ".join(summary)))
