# -*- coding: utf-8 -*-
"""S1 gate call for BS-005 (wrapper around call_expert internals).

Same channel as .bs004-tmp/s1_call.py (R176/R182 proven): reuses call_expert
...
no-output guard does not cancel it. Result -> .bs005-tmp/s1-result.json.
First real-gate run of the S1 v1.5 violation-deduction prompt (backlog #24).
...
MATERIAL = sys.argv[1] if len(sys.argv) > 1 else str(
    REPO / "data" / "sources" / "bs005" / "s1-review-material.md")
RESULT = REPO / ".bs005-tmp" / "s1-result.json"
...
def main():
    ...
    if code == 0 and out.strip():
        ...
        note = "full text=expert-verdicts/%s | %s (1500s wrapper: S1 v1.5 first real gate)" % (vname, first_line)
