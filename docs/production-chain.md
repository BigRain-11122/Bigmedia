...
6. **开闸 checklist**（O-20260924-1043 审计 F2·升格强制·**2026-09-24 D-BS 批全闭**）：①CEO 终审定档→**✅ D-BS-01 自决（v12 全件套）**→②口吻改写批（backlog #15）**✅ 机制+BS-001 首件示范毕**（2026-09-24 21:5X 口径核正：原「已毕」=声线正典机制落定+v11-trim 拍稿首件实证口径；**10 封存稿全量改写=逐件随量产线落地**——余 9 稿盘上未动·逐件 S1 必对声线正典·防假绿灯）→③**首件全链走门演练** **✅ v12 首战走毕（S1-S4 9+·终审 9+）**→④账号批次①就绪→**语义修正（D-BS-06）：=发布前置（M5 门）非生产前置——PLAN §1 P2 原设计即生产先于发布·两闸分离**→⑤BGM 裁决→**✅ D-BS-02 自决（方案 A）**——**四键自决闭环+④发布门独立锁=生产闸已开（production: open·成品库模式·首件 F-001 已登记 `output/finished.md`）**。
7. **成品库**（D-BS-06 新设）：`output/finished.md`——量产件逐件登记（全件套+M4 证据链+发布锁注记）；**S5 发布物**（标题 A/B per title-craft §3-M5、封面、整改面）登记=`output/schedule.md` 发布物登记节。
...
- 2026-09-24: v2.2 生产批一核正+成品库接线——§3 ②「已毕」口径核正（机制+BS-001 首件示范毕·10 封存稿全量改写=逐件随量产线落地·余 9 稿盘上未动）；§3 增第 7 条成品库+S5 发布物落位（`output/finished.md` 循环 R170 立账 F-001+`output/schedule.md` 发布物登记节 bm-a 会话·两账分工：件与证据链=finished.md·发布案物料=schedule.md）。
