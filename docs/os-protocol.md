...
- **idle-fast 收账并窗**（P-62 ③·2026-09-24）：连续空转轮的 commit+push 并窗——**照记律不变**（每轮 state.json tick+log 照写盘=探针数据源·盘面即真相），commit 触发=①窗满 6 轮（≈1 小时）②跨日边界轮 ③五查/探针任一异常（立即收账·恢复逐轮）④出现非 idle 实活轮（实活收账一并卷入）；并窗 commit 消息注明区间（如「R151-R156 idle-fast batch」）；未推增量上限=6 轮。并窗期间 `M src/os/state.json`=本循环自记账预期态——树净判定与 bm-a 活跃写盘判定**排除 state.json 自身**（其余脏文件照常判·同仓退避不误触）。
...
- **全球视野基准面回执（ledger P-20260924-56·本司份额·2026-09-24 会话）**：首版 `docs/global-benchmarks.md` v1.0 落地（四节齐备+9 条执行标准带落地指针与层标注·AIGC 标识办法本仓直采官方全文=A 级锚）——周期条件入任务书铁律（文件 §④ 更新记录首行距今 >7 天=任意轮排刷新轮·采集 ≤15 分钟限时律·零 key 优先·卡点如实）；**转办扫描模式扩展**（`@BigStream` → 增 `@七线全司`/`@全司`——本令即以「@七线全司」下发·原单一模式实测漏扫风险·任务书同步接线）。
