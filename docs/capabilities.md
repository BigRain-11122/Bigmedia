...
| C-24 | 专职专家调用器（call_expert） | 全部门（工程技术部供工具） | live | `python src/call_expert.py --expert <id> --material <文件>`（O-20260924-1033-bm-a）——七部门 11 席名册（`docs/expert-roster.md`·P-62 ② 后八部门并七部门·registry+提示词外置数据件）·本地 Ollama 零 token·**UTF-8 子进程管道=PS5.1 GBK 管道乱码坑根治**（09-23 原生管道实证）·调用自动落 `docs/reviews/expert-calls.md` 台账（行级追加）；6 单测绿·首调实证=hot-intel×2026-09-24 日报（赛道 top3+红线预警+一句话结论·expert-calls 在案） |
