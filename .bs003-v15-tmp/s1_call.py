# -*- coding: utf-8 -*-
"""S1 gate call for BS-003 v15 remake piece (wrapper around call_expert internals).

Same channel as .bs002-v15-tmp/s1_call.py (R449 proven): reuses call_expert
...
no-output guard does not cancel it. Result -> .bs003-v15-tmp/s1-result.json.
...
MATERIAL = sys.argv[1] if len(sys.argv) > 1 else str(
    REPO / "data" / "sources" / "bs003" / "s1-review-material-v15.md")
RESULT = REPO / ".bs003-v15-tmp" / "s1-result.json"
...
def main():
    ...
    if code == 0 and out.strip():
        ...
        note = "full text=expert-verdicts/%s | %s (1500s wrapper: S1 v1.5 gate BS-003 v15 remake)" % (vname, first_line)
