# -*- coding: utf-8 -*-
# R454: frame-sampling probe for bs-004-v15 (three laws: beat heads,
# loop-boundary crossings computed per beat/source + b3 clean-window
# timepoints, full-res text frames)
import json, io, os, subprocess

OUT = '.bs004-v15-tmp/probe-r454'
os.makedirs(OUT, exist_ok=True)
MP4 = 'output/renders/bs-004-v15-shipinhao-60s.mp4'
SRCLEN = {'looplog-vertical.mp4': 12.000, 'reviewsdoc-vertical.mp4': 10.000,
          'citywatch-vertical.mp4': 4.066, 'editgrid-vertical.mp4': 10.000,
          'biggame-cockpit-vertical.mp4': 45.000}
cards = json.load(io.open('data/sources/bs004/cards-v15-matched.json',
...
# 2b) b3 citywatch clean-window timepoints (only citywatch beat; no wrap
# expected since 2.92s < 4.066s source; four timepoints prove zero leak)
b3 = cards[3]
for tag, t in (('a', b3['start'] + 0.15), ('b', b3['start'] + 1.0),
               ('c', b3['start'] + 2.0), ('d', b3['end'] - 0.10)):
    grab(t, 'b03-clean-%s' % tag)
...
tile(['b03-clean-%s' % t for t in 'abcd'], 4,
     os.path.join(OUT, 'tile-b3clean.png'))
if cross:
