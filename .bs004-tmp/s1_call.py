# -*- coding: utf-8 -*-
"""S1 gate call for BS-004 (wrapper around call_expert internals).

Same channel as .bs003-tmp/s1_call.py (R176 proven): reuses call_expert
registry/build_prompt/call_model/verdict-archive/ledger-row verbatim, only
raises subprocess timeout to 1500s so a cold-load + long-prompt eval
cannot be killed at 300s. Run detached (Start-Process) so the 5-min
no-output guard does not cancel it. Result -> .bs004-tmp/s1-result.json.
...
RESULT = REPO / ".bs004-tmp" / "s1-result.json"
