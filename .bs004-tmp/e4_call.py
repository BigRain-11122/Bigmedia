...
# (.bs003-tmp/e4_call.py same-type reuse, R187: path -> bs004, beats -> v6 final, footage mix per cards-v1-matched)
import io, json, os, re, subprocess, time

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)  # repo root = parent of .bs004-tmp
beats_path = os.path.join(ROOT, 'data', 'sources', 'bs004', 'voiceover-v6.beats.txt')
beats = io.open(beats_path, encoding='utf-8').read()

prompt = (
    '你是一名刷短视频的普通观众，刷到下面这条 60 秒竖屏视频（画面全部是真实工作电脑录屏素材：'
    'AI 公司 OS 循环日志、城市值守屏、视频剪辑时间线、像素游戏驾驶舱、评审台账文档，配锚点数字字卡；'
    '配音是轻度赛博机械感的机器叙述者。'
    '口播文案全文如下）：\n\n' + beats + '\n\n请回答三个问题，每题一段，直说不委婉：\n'
